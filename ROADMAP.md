...
Small leftovers: real‑device mobile pass · optional PNG app icons · ~~post‑stay review capture~~ ✅ (2026-07-05: guest reviews with fractional 0.25 star ratings, publish-instantly, migration `11_guest_reviews.sql`).
...
2. **Phase 2.5 — Account/profile page + REAL PAYMENTS**:
   - **2.5a — Account/profile hub** ✅ BUILT (2026-07-05, tsc PASS): shared shell + **Guest⇄Hosting mode switch**; overview (next-trip hero + stats) · trips (upcoming/past) · booking detail (dates/amount/advance/**balance due**/**status timeline**) · **guest self-cancel with policy + auto refund calc** (mirrors CRM `cancelBooking`) · **print-friendly receipt** · balance **interim pay** (bank+screenshot, Safepay slot stubbed) · Profile/Security/Preferences (edit name+phone+re-verify, change password, notification channels + EN/Roman-Urdu) · host-space scaffold. Migration `10_account_prefs.sql`.
   - **2.5b — REAL PAYMENTS** ⬜ NEXT (founder wants payments ASAP): wire Safepay behind `lib/payments/provider.ts` (the "Pay now by card" button is already stubbed in `BalancePanel`).
   - **Real payment gateway** (see memory `payment-integration-plan`): **Safepay** recommended (cards + Easypaisa + JazzCash, best DX/speed) as the "Pay now" option, **kept alongside the current bank-transfer + screenshot** (0% fee — matters on high-ticket stays). Drops in behind `lib/payments/provider.ts` (already built). Guest picks Pay-now (Safepay hosted checkout → webhook → auto `payment_collected`) OR Bank transfer. **Founder: start Safepay onboarding now** (NTN + registration + bank; ~days to approve); I build behind the seam in parallel (~1–2 days).
3b. **Phase 3.5 — Host portal depth** ✅ BUILT (2026-07-08, migration `15_host_portal_2.sql` — ⚠️ needs a full re-run; the `draft` constraint didn't apply): AI listing **interview** (conversational, live-building preview, founder-editable persona), availability **calendar** (host date-blocks → booking clash-check + `public_availability`), **guest info** (property_info + public_facts), host **Bookings** page, **draft**-based creation (photos in-flow), area **dropdown** + city autofill, optional **payout** field. Interview quality-tested vs the real model. Open: photo drag-reorder, host review-response, pricing extras.
3. **Phase 3 + 3.5 + 3.6 — Host portal** ✅ BUILT + DEPLOYED (2026-07-08, migrations `14`/`15`/`16_host_engagement.sql`). 3.6 adds: **host reviews + public reply**, **public "Hosted by…" profile** (name/avatar/since/bio), **listing analytics** (views/inquiries/bookings + 30-day, via a view beacon). owners self-list **free** (commission seam `HOST_COMMISSION_PCT=0`), Esker approves in the CRM, guests book via the **unchanged advance flow**, host handles guest chat (owner-comms routing; staff bell quiet; throttled host email). Depth: **AI listing interview** (conversational, live preview, founder-editable persona), **availability calendar** (host date-blocks → availability + clash-check), **guest info** (private + public facts), host **Bookings** page, **draft**-based creation with an **emphatic photo step** (≥3 required, drag-reorder), **"preview as a guest"**, structured **payout** (all local PK methods). Verified 15/15 + 13/13. Open: payouts ledger, host WhatsApp, review-response, pricing extras.
4. **Phase 4 — Partner + Managed portals**: read-only, relationship-appropriate views; deal math stays server-side.
