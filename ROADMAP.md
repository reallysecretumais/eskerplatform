...
2. **Phase 2.5 — Account/profile page + REAL PAYMENTS** ⬜ NEXT (founder wants payments ASAP):
   - **Account/profile hub** (`app/account`): Profile (name, email-verified state, phone + WhatsApp-verified badge, edit + re-verify, initials avatar) · Bookings upgrade (upcoming vs past, booking-detail view with dates/amount/advance/**balance due**/status timeline, **cancel with policy + refund**, **invoice/receipt download**, rebook, message-about-booking [done]) · Payments (pay balance online, history + receipts, saved method) · Security (change password, sign out) · Preferences (notification channels email/WhatsApp, language EN/Roman-Urdu) · Role shortcuts into Host (P3) / Partner (P4) portals.
