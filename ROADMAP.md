...
- **Quality pass** (commit `28555e8`): SlimListing payload cut (property −21%, AI search −46%) · lazy `<img>` cards with alt/srcset · hero elevation (staggered entrance, serif-gold word, shimmer CTA, cursor gold light, CSS-only sway on mobile, edge mask, scroll cue) · AI **WHY** reasons + refine chips · Help→WhatsApp · featured=6 honest heading · hold reassurance · slot-booking WhatsApp CTA · dead code removed.
