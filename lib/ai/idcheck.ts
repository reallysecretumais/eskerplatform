...
const SYSTEM = `You verify identity documents from a photo for a Pakistani short-stay booking site. The document must be a Pakistani CNIC (national ID card) or a passport.
Return ONLY JSON:
{
  "is_id": boolean,            // true ONLY if the image clearly shows a real CNIC or a passport data page
  "doc_type": "cnic" | "passport" | "other",
  "readable": boolean,         // are the key fields legible?
  "id_number": string|null,    // CNIC (13 digits, dashes ok) or passport number
  "name": string|null,
...
export async function verifyId(file: File): Promise<IdResult> {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: SYSTEM },
          {
            role: "user",
            content: [
              { type: "text", text: `Today is ${today}. Read this ID document.` },
